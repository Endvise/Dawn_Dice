...
def show():
    ...
    09. [회원 관리 (Member Management)](#09-회원-관리-participant-management)
    ...
    **사이드바** → `04. 📊 Dashboard`
    ...
    | **Total Members** | 총 회원 (기존 + 예약) |
    | **Completed Members** | 세션을 완료한 회원 |
    ...
    **사이드바** → `06. 🎯 Session Check-in`
    ...
    ### 💡 팁
    - 대량의 회원를 체크인할 때는 **Quick Check**가 효율적
    - 체크인 상태는 되돌릴 수 없으니 주의
    - 세션 시작 전 미리 회원 목록 확인 권장
    """)
    ...
    **사이드바** → `08. 📋 Reservation Management`
    ...
    # 09. 회원 관리
    # ============================================
    st.markdown("## 09. 회원 관리 (Member Management)")
    st.markdown("---")

    st.markdown("""
    ### 📌 개요
    세션 회원를 관리하고, Excel 파일로 대량 등록할 수 있는 핵심 기능입니다.

    ### 📍 메뉴 위치
    **사이드바** → `09. 👥 Member Management`
    ...
    1. 사이드바에서 `09. 👥 Member Management` 선택
    2. **📥 Import Excel** 탭 선택
    3. **Upload File** 버튼 클릭
    4. Excel/CSV 파일 선택
    ...
    **A:** 다음을 확인하세요:
    ...
    09. [회원 관리](#09-회원-관리-participant-management)
