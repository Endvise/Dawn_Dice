...
## v14 (2026-02-07) - 사용 가이드 페이지 추가

### 변경 내용
1. `admin_user_guide.py` 생성 - 관리자용 한국어 가이드
2. `user_guide_ko.py` 생성 - 일반 사용자용 한국어 가이드
3. `user_guide_en.py` 생성 - 일반 사용자용 영어 가이드
4. `app.py` 모든 메뉴에 01., 02. 형식의 번호 부여
5. `app.py` 새 가이드 페이지 라우팅 추가

### 생성 파일
```
dice_app/views/admin_user_guide.py   # 관리자용 (한국어)
dice_app/views/user_guide_ko.py      # 일반 사용자용 (한국어)
dice_app/views/user_guide_en.py     # 일반 사용자용 (영어)
```
...
## v13 (2026-02-07) - 한국어 이용 안내 공지 추가

### 변경 내용
1. announcements 테이블에 한국어 이용 안내 공지 INSERT
2. 카테고리: guide, 상단 고정: true
3. 포함 내용:
   - 가입 방법 (사령관번호, 닉네임, 서버, 연맹, 비밀번호)
   - 로그인 방법
   - 예약 신청 방법
   - 주의사항 (선착순, 정원 180명, 블랙리스트)

### Supabase SQL
```sql
INSERT INTO announcements (title, content, category, is_pinned, is_active, author_name, created_by)
VALUES ('📖 DaWn Dice Party 이용 안내', '...한국어 내용...', 'guide', true, true, 'Admin', 1);
```
